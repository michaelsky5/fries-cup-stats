...
import SignalTeamDossier from '../../features/team-dossier/SignalTeamDossier.jsx'
import { getArchiveMetricAvailability } from '../../features/team-dossier/teamArchiveContent.js'
import styles from '../../features/fd-design/teamDetailStyles.js'
...
const SHARE_HERO_ROLE_BY_ROSTER_ROLE = {
  TANK: 'tank',
  DPS: 'damage',
  SUP: 'support'
}

function getShareHeroCandidates(player, locale = 'zh-CN') {
  const roleKey = normalizeRosterRole(player?.role)
  const expectedHeroRole = SHARE_HERO_ROLE_BY_ROSTER_ROLE[roleKey]
  const seen = new Set()
  const heroNames = [
    player?.avatar?.heroName,
    ...safeArr(player?.avatar?.heroNames),
    ...safeArr(player?.heroNames),
    player?.most_played_hero,
    ...safeArr(player?.top_3_heroes)
  ]

  return heroNames.reduce((candidates, heroName) => {
    const key = getOwHeroCanonicalKey(heroName)
    if (!key || seen.has(key)) return candidates
    if (expectedHeroRole && getOwHeroRole(heroName) !== expectedHeroRole) return candidates

    const artwork = getShareHeroArtwork(heroName, player?.role)
    seen.add(key)
    candidates.push({
      key,
      name: formatOwHeroName(heroName, locale),
      canonicalName: getOwHeroCanonicalName(heroName),
      avatarSrc: artwork.fallbackSrc,
      hasArtwork: Boolean(artwork.src || artwork.fallbackSrc),
      artwork: {
        src: artwork.src,
...
function buildSharePlayer(player, locale = 'zh-CN') {
  const heroCandidates = getShareHeroCandidates(player, locale)
  const primaryHero = heroCandidates[0]
  const artwork = primaryHero?.artwork || getShareHeroArtwork(player?.avatar?.heroName, player?.role)
  ...
  return {
    ...
    hero: primaryHero?.name || getPlayerHeroLabel(player, locale),
    heroKey: primaryHero?.key || getOwHeroCanonicalKey(player?.avatar?.heroName),
    heroCandidates,
    mapsPlayed,
    timeMins,
    hasAppearance: mapsPlayed > 0 || timeMins > 0,
    avatarSrc: primaryHero?.avatarSrc || getPlayerAvatarSrc(player),
    initials: getPlayerInitials(player),
    hasArtwork: Boolean(artwork.src || artwork.fallbackSrc || primaryHero?.avatarSrc || getPlayerAvatarSrc(player)),
    artwork: {
      src: artwork.src,
      fallbackSrc: artwork.fallbackSrc,
      crop: artwork.crop
...
function getShareModel({
  ...
  return {
    ...
    focusMatch: focusMatch ? {
      kind: advanceState.isArchived ? 'final' : isLiveMatch(focusMatch.match) ? 'live' : isUpcomingMatch(focusMatch.match) ? 'next' : 'latest',
...
function getTeamCombatProfile(players) {
  ...
  return {
    ...
    available: getArchiveMetricAvailability(activePlayers, TEAM_COMBAT_METRICS),
    teamMinutes,
    values
  }
}
...
function getTournamentCombatBaseline(players) {
  ...
  const profiles = [...teams.entries()].map(([id, members]) => ({
    ...getTeamCombatProfile(members),
    id,
    name: members[0].teamShortName || members[0].team_short_name || members[0].team_name || id
  ...
  return { teamCount: profiles.length, values, profiles }
}
...
function getCombatComparisons(profile, baseline, locale = 'zh-CN') {
  ...
  return TEAM_COMBAT_METRICS.map(metric => {
    ...
    const deltaLabel = !ratio
      ? isEn ? 'Awaiting sample' : uiText("样本待生成", locale)
      : delta === 0
        ? isEn ? 'At tournament average' : uiText("持平赛事平均", locale)
        : `${delta > 0 ? '+' : ''}${delta}% ${isEn ? 'vs avg' : uiText("较赛事平均", locale)}`

    return {
      ...metric,
      label: isEn ? metric.labelEn : uiText(metric.labelZh, locale),
      value,
      valueLabel,
      average,
      ratio,
...
function getTeamTacticalProfile(comparisons, mapTypeStats, mapPool, roster, locale = 'zh-CN') {
  ...
  let archetype = isEn ? 'Balanced operations' : uiText("均衡运营型", locale)
  ...
  let description = isEn
    ...
    : uiText("没有单一维度完全主导比赛，队伍更依赖均衡资源分配与稳定的地图执行。", locale)

  if (volatile) {
    archetype = isEn ? 'High-risk trading' : uiText("高风险换血型", locale)
    ...
    description = isEn
      ...
      : uiText("队伍能持续制造高额伤害，但也会给对手留下更多反打窗口，团战选择是首要复盘点。", locale)
  } else if (pressureHigh && defenseHigh) {
    archetype = isEn ? 'Pressure with structure' : uiText("强压控场型", locale)
    ...
    description = isEn
      ...
      : uiText("高于赛事平均的击杀压力由稳定前排承接，队伍能持续掌握主动，同时保留阵型完整性。", locale)
  } else if (pressureHigh) {
    archetype = isEn ? 'High-pressure focus fire' : uiText("高压集火型", locale)
    ...
    description = isEn
      ...
      : uiText("队伍擅长把持续伤害转化为击杀，更倾向通过第一轮集火窗口快速决定团战走向。", locale)
  } else if (defenseHigh) {
    archetype = isEn ? 'Structured sustain' : uiText("稳态阵地型", locale)
    ...
    description = isEn
      ...
      : uiText("前排承压与团队续航是基本盘，队伍在延长团战并逐步控制空间时最具优势。", locale)
  } else if (specialist) {
    archetype = isEn ? 'Map-pool specialist' : uiText("地图专精型", locale)
    ...
    description = isEn
      ...
      : uiText("战绩集中在少数地图类型，熟悉地图上的执行力突出，同时也存在明确的地图池扩展需求。", locale)
  }

  const strengths = strongest.slice(0, 2).map(item => ({
    tone: 'strength',
    label: isEn ? 'Advantage' : uiText("优势", locale),
    title: item.label,
    body: item.ratio
      ? `${item.ratio >= 1 ? '+' : ''}${Math.round((item.ratio - 1) * 100)}% ${isEn ? 'vs tournament baseline' : uiText('较赛事基线', locale)}`
      : isEn ? 'Awaiting sample' : uiText("样本待生成", locale)
  }))
  const risks = []
  if (specialist) {
    risks.push({
      tone: 'risk',
      label: isEn ? 'Risk' : uiText("风险", locale),
      title: isEn ? 'Narrow mode coverage' : uiText("地图类型覆盖偏窄", locale),
      body: isEn ? `${mapTypeStats.length} modes currently tracked` : uiText("当前仅覆盖 {0} 类地图模式", locale, [mapTypeStats.length])
    })
  }
  if (signature && signatureShare >= 0.4) {
    risks.push({
      tone: 'risk',
      label: isEn ? 'Dependency' : uiText("依赖", locale),
      title: isEn ? `${signature.displayName} concentration` : uiText("{0}样本集中", locale, [signature.displayName]),
      body: `${formatPercent(signatureShare)} ${isEn ? 'of tracked maps' : uiText('的地图样本来自该图', locale)}`
    })
  }
  if (!risks.length && weakest[0] && weakest[0].score < 48) {
    risks.push({
      tone: 'risk',
      label: isEn ? 'Review' : uiText("复盘", locale),
      title: weakest[0].label,
      body: `${Math.round((weakest[0].ratio - 1) * 100)}% ${isEn ? 'vs tournament baseline' : uiText('较赛事基线', locale)}`
    })
  }
  const plan = specialist
    ? isEn ? 'Prioritize one additional map mode in training while preserving the current signature-map structure.' : uiText("训练优先补齐一种新地图类型，同时保留现有招牌地图体系。", locale)
    : pressureHigh
      ? isEn ? 'Build set plays around the first focus-fire window and review disengage timing after failed entries.' : uiText("围绕第一轮集火窗口设计固定战术，并重点复盘进场失败后的脱离时机。", locale)
      : isEn ? 'Use the weakest tactical axis as the next training-cycle objective and verify it across multiple map modes.' : uiText("把当前最弱战术维度设为下一训练周期目标，并在多种地图类型中验证。", locale)
  ...
  return {
    ...
    description,
    axes,
    notes: [...strengths, ...risks.slice(0, 1), { tone: 'plan', label: isEn ? 'Training' : uiText("训练建议", locale), title: isEn ? 'Next focus' : uiText("下一步重点", locale), body: plan }],
    confidence: mapSamples >= 8 && validAxes.length >= 5 ? (isEn ? 'HIGH' : uiText("高", locale)) : mapSamples >= 4 ? (isEn ? 'MEDIUM' : uiText("中", locale)) : (isEn ? 'LOW' : uiText("低", locale)),
    mapSamples,
...
function getPlayerStyleProfiles(players, allPlayers, locale = 'zh-CN') {
  ...
  return safeArr(players).map(player => {
    ...
    let style = best ? (isEn ? best.labelEn : uiText(best.labelZh, locale)) : (isEn ? 'Role sample pending' : uiText("职责样本待生成", locale))
...
function FormStrip({ form }) {
  ...
  if (!form.length) return <span className={styles.formEmpty}>{uiText("暂无赛果", uiLocale)}</span>
  ...
  return (
    ...
    >
      ...
      <span className={styles.focusMatchHint}>
        ...
        <b>{isPending ? uiText("赛前数据待更新", uiLocale) : uiText("最近完成比赛", uiLocale)}</b>
      </span>
      <span className={styles.focusMatchPrep}>
        <span>
          <em>{uiText("开赛状态", uiLocale)}</em>
          <b>{statusLabel}</b>
        </span>
        <span>
          <em>{uiText("近期状态", uiLocale)}</em>
          <FormStrip form={recentForm} />
        </span>
      </span>
      <span className={styles.focusMatchStats}>
        <span><b>{recordLabel}</b><em>{uiText("比赛战绩", uiLocale)}</em></span>
        <span><b>{mapRecordLabel}</b><em>{uiText("地图战绩", uiLocale)}</em></span>
        <span><b>{pendingLabel}</b><em>{uiText("待赛", uiLocale)}</em></span>
      </span>
    </Link>
  )
}
...
function RosterPlayerArtwork({ player }) {
  const heroName = player?.avatar?.heroName
  const artworkRole = getOwHeroRole(heroName) || player?.role
  const artwork = getShareHeroArtwork(heroName, artworkRole)
      ...
      <span className={styles.seasonMatchMaps}>
        ...
        <span className={styles.seasonMatchMapChips}>
          {maps.length ? maps.map((map, mapIndex) => {
            ...
            <span className={styles.seasonMatchMapEmpty}>{isPending ? uiText("地图待更新", locale) : uiText("暂无地图明细", locale)}</span>
          )}
        </span>
      </span>
...
function CombatMetricCard({ item, isEn }) {
  ...
  return (
    <article className={styles.combatMetricCard} data-tone={item.tone}>
      ...
      <small className={styles.combatMetricFoot}>{isEn ? 'Tournament average' : uiText("赛事平均", uiLocale)} {item.average ? formatRosterPerformanceValue(item.average) : '--'}</small>
    </article>
  )
}
...
function TacticalAxisRow({ axis, isEn }) {
  ...
  return (
    <span className={styles.tacticalAxisRow} data-tone={axis.score >= 56 ? 'high' : axis.score && axis.score < 46 ? 'low' : 'average'}>
      ...
      <small>{isEn ? 'TOURNAMENT BASELINE' : uiText("赛事基线", uiLocale)}</small>
    </span>
  )
}
  ...
  return (
    <article className={styles.recordedCompCard} data-rank={lineup.rank}>
      <header className={styles.recordedCompCardHead}>
        <span>
          <em>{isEn ? 'RECORDED COMP' : uiText("记录阵容", uiLocale)}</em>
        ...
        <span className={styles.recordedCompCardShare}>
          ...
          <em>{isEn ? 'SHARE' : uiText("记录占比", uiLocale)}</em>
        </span>
      </header>
      ...
      <footer className={styles.recordedCompCardMeta}>
        <span>
          <em>{isEn ? 'COMMON MAP' : uiText("常见地图", uiLocale)}</em>
          <strong title={lineup.commonMap}>{lineup.commonMap}</strong>
        </span>
        <span>
          <em>{isEn ? 'RECORDED' : uiText("记录次数", uiLocale)}</em>
          <strong>{lineup.maps}</strong>
        </span>
        <span>
          <em>{isEn ? 'MAP RECORD' : uiText("地图战绩", uiLocale)}</em>
  ...
  return (
    <article className={styles.playerStyleCard} data-role={profile.role}>
      ...
      <span className={styles.playerStyleReadout}>
        <em>{isEn ? 'ROLE READ' : uiText("职责画像", uiLocale)}</em>
      ...
      <span className={styles.playerStylePercentile}>
        ...
        <em>{isEn ? 'ROLE PERCENTILE' : uiText("同职责百分位", uiLocale)}</em>
      </span>
    </article>
  )
}
  ...
  return (
    <article className={styles.mapModeRow}>
      <span className={styles.mapModeIdentity}>
        ...
        <em>{row.maps} {isEn ? 'maps' : uiText("张地图", locale)}</em>
      </span>
  ...
  return (
    <article className={styles.teamMapCard} data-tone={map.tag.tone}>
      <span className={styles.teamMapVisual}>
        ...
        />
        <em>{isEn ? map.tag.labelEn : uiText(map.tag.labelZh, uiLocale)}</em>
      </span>
      <span className={styles.teamMapInfo}>
        <small>{map.mode}</small>
        <strong>{map.displayName}</strong>
        <span className={styles.teamMapMetrics}>
          <span><b>{map.recordLabel}</b><em>{isEn ? 'Record' : uiText("地图战绩", uiLocale)}</em></span>
          <span><b>{formatPercent(map.winRate)}</b><em>{isEn ? 'Win rate' : uiText("地图胜率", uiLocale)}</em></span>
          <span><b>{map.maps}</b><em>{isEn ? 'Played' : uiText("出场次数", uiLocale)}</em></span>
        </span>
        <span className={styles.teamMapLatest} data-tone={map.latestTone}>
          <b>{latestLabel}</b>
          <em>{isEn ? 'Latest' : uiText("最近", uiLocale)} vs {map.latestOpponent || 'TBD'}</em>
        </span>
      </span>
    </article>
  )
...
function RosterPlayerRow({ player, withSeason, locale = 'zh-CN', isFdDesign = false }) {
  ...
  return (
    <Link to={withSeason(`/players/${player.identity?.playerId || player.player_id}`)} className={styles.rosterPlayerCard}>
      {isFdDesign ? (
        <HeroArtwork hero={player.avatar?.heroName || player.most_played_hero} className={styles.rosterPlayerArtwork} decorative locale={locale} />
      ...
      <span className={`${styles.rosterPlayerPerformance} ${performanceMetric ? '' : styles.rosterPlayerPerformanceEmpty}`}>
        <em>{performanceMetric?.label || uiText("角色表现", locale)}</em>
        <span>
          <b>{performanceMetric?.valueLabel || '--'}</b>
          <small>{performanceMetric?.unit || uiText("待更新", locale)}</small>
        </span>
      </span>
      {hasUsage ? (
        <span className={styles.rosterPlayerMeta}>
          <span><b>{mapsPlayed}</b><em>{uiText("出场地图", locale)}</em></span>
          <span><b>{formatPlayerTime(timeMins)}</b><em>{uiText("出场时长", locale)}</em></span>
        </span>
      ) : (
        <span className={styles.rosterPlayerMeta}>
          <span className={styles.rosterPlayerMetaEmpty}>{uiText("赛时数据待更新", locale)}</span>
        </span>
      )}
    </Link>
  )
...
export default function TeamDetailPage() {
  ...
  const [signalShareSummary, setSignalShareSummary] = useState(null)
  const matchFilter = searchParams.get('result') || 'all'
  const setMatchFilter = value => {
    const next = new URLSearchParams(searchParams)
    if (value === 'all') next.delete('result')
    else next.set('result', value)
  ...
  const groupCompetition = useMemo(() => isGroupCompetition(season, db), [season, db])
  const standings = useMemo(() => (
    groupCompetition
      ? getGroupStandings(db, season).flatMap(group => group.rows)
      : getSwissStandingsRows(db, season)
  ), [db, season, groupCompetition])
  ...
  const matchFilterOptions = [
    { id: 'all', label: uiText("全部", locale), count: matchRows.length },
    { id: 'win', label: uiText("胜场", locale), count: wonRows.length },
    { id: 'loss', label: uiText("负场", locale), count: lostRows.length },
    { id: 'pending', label: uiText("待进行", locale), count: upcomingRows.length }
  ]
  ...
  const advanceState = useMemo(() => getTeamSeasonPresentation({ db, season, team, standing, groupCompetition, locale }), [db, season, team, standing, groupCompetition, locale])
  ...
  const focusMatch = todayRows.find(row => isLiveMatch(row.match) || isUpcomingMatch(row.match)) ||
    ...
    finishedRows[0] ||
    null
  const focusCode = advanceState.isArchived ? 'FINAL APPEARANCE' : focusMatch && isLiveMatch(focusMatch.match) ? 'LIVE MATCH' : nextMatch ? 'NEXT MATCH' : 'LATEST MATCH'
    ...
    return (
      <div className={styles.shell}>
        <section className={styles.errorState}>
          ...
          <h1>{uiText("未找到战队", locale)}</h1>
          <p>{uiText("当前赛季中不存在编号或简称为 ", locale)}{teamId}{uiText(" 的战队。", locale)}</p>
          <button type="button" onClick={handleBack} className={styles.primaryButton}>{uiText("返回战队目录", locale)}</button>
        </section>
      </div>
    )
  }
  ...
  if (isKprHybridDesign) {
    return <>
      <SignalTeamDossier
        ...
        onShare={summary => { setSignalShareSummary(summary); setShareOpen(true) }}
        onToggleFavorite={() => toggleTeamFavorite?.(team)}
        favorited={teamFavorited} favoriteDisabled={favoriteLimitReached}
        sourceReturnState={sourceReturnState}
      />
      <TeamShareDialog open={shareOpen} onClose={() => setShareOpen(false)} model={{ ...teamShareModel, ...signalShareSummary }} />
    </>
  }
  ...
  return (
    <div className={styles.shell}>
      <div className={styles.topbar}>
        <button type="button" className={styles.backButton} onClick={handleBack}>{uiText("← 返回", locale)}</button>
        <Link to={withSeason('/me?section=following&manage=1')} className={styles.manageLink}>{uiText("管理关注 →", locale)}</Link>
      </div>

      {!isKprDesign ? <RosterSubnav withSeason={withSeason} /> : null}
      ...
      <section className={styles.teamHero}>
          ...
          <div className={styles.heroMetaStrip}>
            ...
            <span className={styles.heroRosterMetaItem}>
              ...
              <b className={styles.heroRolePills}>
                ...
                )) : uiText("{0} 名选手", locale, [roster.length])}
              </b>
            </span>
          </div>

          <div className={styles.heroMetrics}>
            <HeaderMetric label={advanceState.heading} value={advanceState.label} meta={advanceState.zone} />
            <HeaderMetric label={uiText("比赛战绩", locale)} value={`${matchSummary.wins}-${matchSummary.losses}`} meta={`${matchSummary.completed} 场已完成`} />
            <HeaderMetric label={uiText("比赛胜率", locale)} value={formatPercent(matchSummary.winRate)} meta={`${matchSummary.wins}-${matchSummary.losses}`} />
          </div>
        </div>
        ...
        <aside className={styles.heroIntelStage}>
          <div className={styles.heroActions}>
            ...
            >
              {teamFavorited ? uiText("取消关注", locale) : favoriteLimitReached ? uiText("关注已满", locale) : uiText("关注战队", locale)}
            </button>
            <button
              type="button"
              className={styles.shareButton}
              onClick={() => setShareOpen(true)}
            >{uiText("导出分享图", locale)}</button>
          </div>

          <div className={styles.focusDossier}>
            <span>{focusCode}</span>
            <strong>{focusMatch ? focusMatch.opponentLabel : advanceState.isArchived ? (isEn ? 'Season archived' : uiText("赛季已归档", locale)) : (isEn ? 'Opponent pending' : uiText("对手待定", locale))}</strong>
            <em>{focusMatch ? `${getMatchRound(focusMatch.match)} · ${getMatchTimeLabel(focusMatch.match)}` : (isEn ? 'No published match' : uiText("暂无已发布比赛", locale))}</em>
            {focusMatch ? <>
              <b>{focusMatch.score}</b>
              <Link to={withSeason(`/matches/${encodeURIComponent(focusMatch.match.match_id)}`)} state={getReturnState(location)} onClick={() => saveReturnScroll(location)}>
                {isEn ? 'Open match record' : uiText("查看比赛记录", locale)} ↗
              </Link>
            </> : null}
          </div>
          ...
          <div className={styles.heroIntelFacts}>
            <span>
              ...
              <em>{uiText("地图战绩", locale)}</em>
            </span>
            <span>
              <strong>{formatPercent(matchSummary.mapWinRate)}</strong>
              <em>{uiText("地图胜率", locale)}</em>
            </span>
            <span>
              <strong>{advanceState.isArchived ? matchSummary.completed : todayRows.length || '—'}</strong>
              <em>{advanceState.isArchived ? (isEn ? 'Completed matches' : uiText("完成场次", locale)) : uiText("今日赛程", locale)}</em>
            </span>
            <span>
              <strong>{advanceState.isArchived ? (isEn ? 'Archived' : uiText("已归档", locale)) : matchSummary.pending || '—'}</strong>
              <em>{advanceState.isArchived ? (isEn ? 'Season status' : uiText("赛季状态", locale)) : uiText("待赛", locale)}</em>
            </span>
          </div>
        </aside>
      </section>

      <div className={styles.tabs} role="group" aria-label={isEn ? 'Team dossier views' : uiText("队伍档案视图", locale)}>
        {TABS.map(tab => (
          <button
            key={tab.id}
            type="button"
          ...
          >
            {isEn ? ({ overview: 'Overview', matches: 'Matches', stats: 'Team stats' })[tab.id] : uiText(tab.label, locale)}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'overview' ? (
        <div className={styles.overviewGrid}>
            ...
            <div className={styles.mapSpotlight}>
              <div className={styles.mapSpotlightVisual}>
                ...
                <div className={styles.mapSpotlightCaption}>
                  ...
                  <em>{featuredMap.hasData ? `${featuredMap.mode} · ${featuredMap.maps} maps tracked` : uiText("暂无有效地图记录", locale)}</em>
                </div>
              </div>
              <div className={styles.mapSpotlightIntel}>
                ...
                <div className={styles.mapIntelGrid}>
                  ...
                  <span>
                    ...
                    <em>{featuredMap.typeRecord ? uiText("{0} 类型", locale, [featuredMap.typeRecord.mode]) : uiText("类型战绩", locale)}</em>
                  </span>
                </div>
                <div className={styles.mapIntelNote}>
                  ...
                  {featuredMap.hasData ? (
                    ...
                    <strong>{uiText("等待已完赛地图数据", locale)}</strong>
                  )}
          ...
          <section className={`${styles.panel} ${styles.corePanel}`}>
            <div className={styles.panelHead}>
              <h2>{isEn ? 'Featured players' : uiText("出场核心", locale)}</h2>
              <span>{isEn ? 'BASED ON SEASON APPEARANCES' : uiText("按赛季出场记录筛选", locale)}</span>
            </div>
              ...
              {startingFiveSlots.map((slot, index) => {
                  ...
                  return (
                    <div key={slot.id} className={`${styles.coreRosterCard} ${styles.coreRosterEmpty}`}>
                      ...
                      <span className={styles.coreRosterName}>
                        <strong>{uiText("待定", locale)}</strong>
                        <em>{uiText("阵容待补充", locale)}</em>
                      </span>
                    </div>
                  )
                }
                ...
                return (
                  ...
                  >
                    {isFdDesign ? (
                      <>
                        <HeroArtwork hero={player.avatar?.heroName || player.most_played_hero} className={styles.coreHeroArt} decorative locale={locale} />
                        <span className={styles.coreHeroCaption}>{getPlayerHeroLabel(player, locale)}</span>
                      </>
                    ) : <PlayerPortrait player={player} className={styles.coreRosterHeroWash} decorative />}
                    <span className={styles.coreSlotTop}>
                      <b>{getRosterRoleLabel(player.role, locale)}</b>
                      <em>{String(index + 1).padStart(2, '0')}</em>
                    </span>
                    ...
                    {hasUsage ? (
                      <span className={styles.coreRosterMeta}>
                        <span><b>{formatPercent(appearanceRate)}</b><em>{uiText("出场率", locale)}</em></span>
                        <span><b>{formatPlayerTime(timeMins)}</b><em>{uiText("出场时长", locale)}</em></span>
                      </span>
                    ) : (
                      <span className={styles.coreRosterMeta}>
                        <span className={styles.coreRosterMetaEmpty}>{uiText("赛时待更新", locale)}</span>
                      </span>
                    )}
                  </Link>
                )
            ...
            <div className={styles.scheduleBoard}>
              <div className={styles.scheduleFocus}>
                {focusMatch ? (
                  ...
                  <div className={styles.emptyPanel}>{uiText("暂无待进行比赛", locale)}</div>
                )}
              </div>
              <div className={styles.scheduleRecent}>
                <div className={styles.scheduleSubhead}>
                  <strong>{uiText("最近赛果", locale)}</strong>
                ...
                <div className={styles.recentTimeline}>
                  ...
                  )) : <div className={styles.emptyPanel}>{uiText("暂无已完成比赛", locale)}</div>}
                </div>
              </div>
            </div>
          </section>
      ...
      {activeTab === 'matches' ? (
        <section className={`${styles.panel} ${styles.matchesPanel}`}>
          <div className={styles.panelHead}>
            <h2>{uiText("赛程赛果", locale)}</h2>
            <Link to={withSeason(`/matches?view=list&tab=all&team=${encodeURIComponent(team.shortName)}&teamId=${encodeURIComponent(team.routeId)}`)}>{uiText("查看全部相关比赛 →", locale)}</Link>
          </div>
          <div className={styles.matchLedgerSummary}>
            <span><em>SEASON PROGRESS</em><strong>{seasonLedgerStats.progress}</strong><small>{uiText("赛程进度", locale)}</small></span>
            <span><em>MAP DIFFERENTIAL</em><strong>{seasonLedgerStats.mapDifferential}</strong><small>{uiText("净胜地图", locale)}</small></span>
            <span><em>SWEEPS</em><strong>{seasonLedgerStats.sweeps}</strong><small>{uiText("横扫场次", locale)}</small></span>
            <span>
              <em>WIN STREAK</em>
              <strong>W{seasonLedgerStats.currentWinStreak}</strong>
              <small>{advanceState.isArchived ? uiText("收官连胜", locale) : uiText("当前连胜", locale)}{uiText(" · 最长 W", locale)}{seasonLedgerStats.longestWinStreak}</small>
            </span>
          </div>
          <div className={styles.matchLedgerToolbar}>
            <div>
              <span>SEASON LEDGER</span>
              <strong>{uiText("完整赛季记录", locale)}</strong>
            </div>
            <div className={styles.matchFilters} role="group" aria-label={uiText("赛程筛选", locale)}>
          ...
          <div className={styles.matchLedgerList}>
            {filteredMatchRows.length ? filteredMatchRows.map((row, index) => (
              <TeamSeasonMatchRow
                ...
                locale={locale}
              />
            )) : <div className={`${styles.emptyPanel} ${styles.matchLedgerEmpty}`}>{uiText("当前筛选下暂无比赛", locale)}</div>}
          </div>
        </section>
      ) : null}
      ...
      {activeTab === 'stats' ? (
        <section className={`${styles.panel} ${styles.statsPanel}`}>
          <div className={styles.panelHead}>
            <h2>{isEn ? 'Team Stats' : uiText("战队数据", locale)}</h2>
            <span>{isEn ? 'TACTICAL PROFILE' : uiText("战术画像", locale)}</span>
          </div>
          ...
          <section className={styles.tacticalCommandDeck}>
            <div className={styles.tacticalIdentity}>
              <span className={styles.tacticalEyebrow}>
                ...
                <small>{isEn ? 'Data-derived tactical read' : uiText("基于赛事数据的战术研判", locale)}</small>
              </span>
              ...
              <span className={styles.tacticalMetaStrip}>
                <span><em>{isEn ? 'CONFIDENCE' : uiText("研判可信度", locale)}</em><strong>{tacticalProfile.confidence}</strong></span>
                <span><em>{isEn ? 'MAP SAMPLE' : uiText("地图样本", locale)}</em><strong>{tacticalProfile.mapSamples}</strong></span>
                <span><em>{isEn ? 'MODE COVERAGE' : uiText("模式覆盖", locale)}</em><strong>{tacticalProfile.modeCount}/{Object.keys(OW_MAP_MODE_LABELS).length}</strong></span>
                <span><em>{isEn ? 'CORE LOAD' : uiText("主力负载", locale)}</em><strong>{tacticalProfile.coreShare ? formatPercent(tacticalProfile.coreShare) : '--'}</strong></span>
              </span>
            </div>
            ...
            <div className={styles.tacticalAxes}>
              <span className={styles.tacticalSubhead}>
                <em>TACTICAL DNA</em>
                <strong>{isEn ? 'Offense and defense tendencies' : uiText("攻防倾向", locale)}</strong>
              </span>
          ...
          <section className={styles.recordedCompsSection}>
            <div className={styles.statsSectionHead}>
              <span>
                ...
                <h3>{isEn ? 'Recorded five-hero lineups' : uiText("最终记录阵容", locale)}</h3>
              </span>
              <small>
                {isEn
                  ? `TOP 3 · ${recordedLineups.completeSamples} COMPLETE SAMPLES`
                  : uiText("TOP 3 · {0} 个完整样本", locale, [recordedLineups.completeSamples])}
              </small>
            </div>
            {recordedLineups.lineups.length ? (
              <>
                ...
                <div className={styles.recordedCompsMethod}>
                  <strong>{isEn ? 'METHOD' : uiText("统计口径", locale)}</strong>
                  <span>
                    {isEn
                      ? 'Built from each map\'s final recorded five-hero set; this does not imply full-map uptime.'
                      : uiText("基于每张地图最终记录的五英雄组合统计，不代表整局全程阵容。", locale)}
                  </span>
                </div>
              </>
            ) : (
              <div className={`${styles.emptyPanel} ${styles.recordedCompsEmpty}`}>
                <strong>{isEn ? 'No complete recorded lineup yet' : uiText("暂无完整记录阵容", locale)}</strong>
                <span>{isEn ? 'This section appears after all five final heroes are recorded on the same map.' : uiText("同一张地图记录完整五个最终英雄后将在这里生成。", locale)}</span>
              </div>
            )}
          </section>
          ...
          <section className={styles.coachingWorkbench}>
            <div className={styles.statsSectionHead}>
              <span>
                ...
                <h3>{isEn ? 'Coaching observations' : uiText("教练观察", locale)}</h3>
              </span>
              <small>{isEn ? 'Strengths, exposure and the next training focus' : uiText("优势、暴露点与下一训练重点", locale)}</small>
            </div>
          ...
          <section className={styles.statsDataSection}>
            <div className={styles.statsSectionHead}>
              <span>
                ...
                <h3>{isEn ? 'Team combat profile' : uiText("团队作战画像", locale)}</h3>
              </span>
              <small>
                {teamCombatProfile.hasData
                  ? `${isEn ? 'Compared with' : uiText("对比", locale)} ${tournamentCombatBaseline.teamCount} ${isEn ? 'teams' : uiText("支有效样本队伍", locale)}`
                  : isEn ? 'Awaiting official combat data' : uiText("等待官方赛时作战数据", locale)}
              </small>
            </div>
          ...
          <section className={styles.statsDataSection}>
            <div className={styles.statsSectionHead}>
              <span>
                ...
                <h3>{isEn ? 'Core player role reads' : uiText("核心选手职责画像", locale)}</h3>
              </span>
              <small>{isEn ? 'Strongest role-relative trait for each starter' : uiText("展示每名首发相对同职责最突出的维度", locale)}</small>
            </div>
            ...
            <section className={styles.statsDataSection}>
              <div className={styles.statsSectionHead}>
                <span>
                  ...
                  <h3>{isEn ? 'Map mode performance' : uiText("地图类型表现", locale)}</h3>
                </span>
                <small>{mapPoolSamples} {isEn ? 'maps tracked' : uiText("张地图样本", locale)}</small>
              </div>
              ...
              {mapTypeStats.length ? mapTypeStats.map(row => (
                ...
                )) : <div className={`${styles.emptyPanel} ${styles.statsEmptyPanel}`}>{isEn ? 'No map mode data' : uiText("暂无地图类型数据", locale)}</div>}
              </div>
            </section>
            ...
            <section className={`${styles.statsDataSection} ${styles.mapPoolSection}`}>
              <div className={styles.statsSectionHead}>
                <span>
                  ...
                  <h3>{isEn ? 'Complete map pool' : uiText("完整地图池", locale)}</h3>
                </span>
                <small>{teamMapPool.length} {isEn ? 'maps' : uiText("张地图", locale)}</small>
              </div>
              <div className={styles.teamMapGrid}>
                {teamMapPool.length ? teamMapPool.map(map => (
                  <TeamMapPoolCard key={map.name} map={map} isEn={isEn} />
                )) : <div className={`${styles.emptyPanel} ${styles.statsEmptyPanel}`}>{isEn ? 'No map pool data' : uiText("暂无地图池数据", locale)}</div>}
              </div>
            </section>
          </div>
        </section>
