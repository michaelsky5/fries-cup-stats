...
import {
  ScoutingDossierSnapshot,
  ScoutingReportAudience as SharedScoutingReportAudience,
  ScoutingReportHeader as SharedScoutingReportHeader,
  ScoutingReportHero as SharedScoutingReportHero,
  ScoutingReportVersion as SharedScoutingReportVersion,
...
const COPY = {
  'zh-CN': {
    ...
    comparisonAuditTableMeta: '保留所有精确数值供教练复核；默认折叠，经理视图优先显示结论与差异。',
    comparisonNeedTwo: '请至少选择 2 名同位置选手以生成决策结论。',
    deploymentMatrix: '位置使用地图',
    deploymentMatrixMeta: '五个位置分别成图；散点只比较日常基线与高压准备度，英雄和地图环境迁移性拆到右侧排行榜，不再依赖难比较的气泡大小。',
    ...
    roleMedian: '同职责中位',
    loading: '正在载入技术分析数据…',
    loadingDetail: '正在读取已发布的轻量索引；选手完整档案将在进入详情页后按需载入。',
    loadingPlayer: '正在载入选手完整档案…',
    loadingPlayerDetail: '档案数据与总览分离载入，仅传输当前选手的分析证据。',
    error: '技术分析数据暂时无法载入。',
    ...
    methodologyItems: [
      ...
      '四种用人侧重先在完整 34 人合格池内按细分位置计算，再标出公开 5 人中的顺位；页面同时展示完整池顺位和公开名单顺位，避免在截断名单后才发现专项候选。',
  ...
  'en-US': {
    ...
    comparisonAuditTableMeta: 'Exact values remain available for coaching review; the manager view leads with the decision and visible differences.',
    ...
    deploymentMatrixMeta: 'Each position has its own map. The scatter compares only everyday baseline and pressure readiness; hero/map portability moves to an explicit ranking instead of hard-to-compare bubble sizes.',
    ...
    loadingDetail: 'Reading the lightweight published index. Full dossiers load only when opened.',
    loadingPlayer: 'Loading full player dossier…',
    loadingPlayerDetail: 'Dossier data is separate from the overview and transfers evidence for this player only.',
    ...
    methodologyItems: [
      ...
      'All four selection emphases are calculated inside the complete 34-player qualified pool before the public five are displayed. The page exposes both full-pool and published-shortlist rank, preventing a specialist from disappearing only because the list was cut first.',
  ...
  'ko-KR': {
    ...
    comparisonAuditTableMeta: '코치 검토용 정확한 수치는 유지하되 매니저 화면은 결론과 차이를 먼저 보여 줍니다.',
    comparisonNeedTwo: '의사결정 해석을 만들려면 같은 세부 역할 선수 2명 이상을 선택하세요.',
    deploymentMatrix: '포지션 기용 지도',
    deploymentMatrixMeta: '5개 포지션을 각각 표시합니다. 산점도는 일상 기준선과 압박 준비도만 비교하고, 영웅·전장 환경 전환성은 버블 크기 대신 별도 순위로 보여 줍니다.',
    ...
    loadingDetail: '게시된 경량 인덱스를 읽는 중입니다. 전체 선수 프로필은 상세 페이지에서만 불러옵니다.',
    loadingPlayer: '선수 전체 프로필을 불러오는 중…',
    loadingPlayerDetail: '전체 명단과 분리된 데이터에서 현재 선수의 분석 근거만 전송합니다.',
    ...
    methodologyItems: [
      ...
      '네 가지 평가 초점은 공개 5인을 자르기 전에 전체 34인 유효 후보군에서 먼저 계산합니다. 화면에는 전체 후보군 순위와 공개 명단 순위를 함께 표시해 전문 후보가 조기 절단으로 사라지는 문제를 확인할 수 있습니다.',
...
function getManagerComparisonCopy(locale) {
  if (locale === 'en-US') {
    return {
      ...
      snapshotMeta: 'Four direct answers lead the page; the weighted factors and exact metrics remain below for technical review.',
  ...
  if (locale === 'ko-KR') {
    return {
      ...
      snapshotMeta: '네 가지 직접 답변을 먼저 제시하고 가중 요인과 정확한 지표는 기술 검토용으로 아래에 유지합니다.',
  ...
  return {
    ...
    snapshotMeta: '先直接回答四个招聘问题；加权因素与精确指标继续保留在下方，供教练和分析人员复核。',
...
function getEvidenceLocatorCopy(locale) {
  if (locale === 'en-US') return {
    eyebrow: 'DECISION EVIDENCE LOCATOR',
    title: 'Jump from the conclusion to its evidence',
    meta: 'Each signal opens the relevant metric, opponent or match evidence instead of asking the reader to search the dossier.',
  ...
  if (locale === 'ko-KR') return {
    eyebrow: 'DECISION EVIDENCE LOCATOR',
    title: '결론에서 근거로 바로 이동',
    meta: '지표, 강팀, 최근 경기 근거를 리포트 안에서 바로 찾아갑니다.',
    metric: '동일 역할 지표',
    opponent: '압박 경기 근거',
    recent: '최근 흐름 근거',
    open: '근거 보기'
  ...
  return {
    eyebrow: 'DECISION EVIDENCE LOCATOR',
    title: '从结论直达证据',
    meta: '不再让读者自行翻找；每个信号直接定位到对应指标、强敌或近期比赛证据。',
    metric: '同职责指标',
    opponent: '强敌与压力证据',
    recent: '近期状态证据',
    open: '查看证据'
...
function getDisclosureCopy(locale) {
  if (locale === 'en-US') return {
    ...
    rankMeta: 'The comparison desk above already carries the decision. Expand only when you need every candidate across all four emphases.',
  ...
  if (locale === 'ko-KR') return {
    ...
    rankMeta: '위 비교 데스크가 핵심 결론을 담고 있습니다. 네 가지 평가 초점의 전체 후보 순위가 필요할 때만 펼치세요.',
    auditTitle: '모델 변환 감사 열기',
    auditMeta: '원점수 → 맥락 보정 → 신뢰도 축소 → 최종 선발 점수를 추적합니다.'
  }
  return {
    rankTitle: '展开完整需求敏感度排名',
    rankMeta: '上方对比台已承载主要结论；只有需要核对四种用人侧重下的完整候选顺序时再展开。',
    auditTitle: '展开模型变换审计',
...
function ManagerPlayerSnapshot({ player, locale, activeScenario, decision }) {
  ...
  return <ScoutingDossierSnapshot
    color={SUBROLE_COLORS[player.subrole]}
    eyebrow={copy.snapshotEyebrow}
    title={copy.snapshotTitle(player.identity.displayName)}
    badge={decision?.archetype || getCandidateStatusLabel(player, locale)}
    summary={decision?.managerSummary || decision?.verdict || getVerdict(player, locale)}
    positionLabel={copy.snapshotPosition}
    positionValue={scenarioPositionLabel}
    rank={fit?.rank ?? player.highSampleSubroleRank}
    total={fit?.total ?? player.highSampleSubroleTotal}
    positionMeta={`${scenarioCopy.label} · ${fit?.score ?? '—'} FIT`}
    lenses={lenses.map(lens => {
      const band = getDecisionProfileBand(lens.value, locale)
      return {
        ...lens,
        value: formatComparisonValue(lens.value),
        numericValue: lens.value,
        level: band.level,
        bandLabel: band.label,
        footer: copy.snapshotScale
      }
    })}
    footerItems={[
      {
        id: 'best-use',
        label: copy.snapshotBestUse,
        content: primaryUse ? (
          <div>
      ...
      { id: 'first-check', label: copy.snapshotFirstCheck, content: <p>{firstCheck}</p> }
    ]}
  />
}
...
function RoleRecruitmentCockpit({ model, locale, audience = 'manager', scenario, subrole, onScenarioChange, onRoleChange, getPlayerHref, returnTo }) {
  ...
  return (
    <section id="role-cockpit" className={styles.roleCockpit} style={{ '--slot-color': SUBROLE_COLORS[subrole] }} aria-labelledby="role-cockpit-title" tabIndex={-1}>
      <ScoutingRoleHeader
        eyebrow={`00 · ${getSubroleLabel(subrole, 'en-US')} SCOUTING BRIEF`}
        title={`${getSubroleLabel(subrole, locale)} · ${t.roleCockpit}`}
        titleId="role-cockpit-title"
        meta={getRoleCockpitMeta(subrole, players.length, locale)}
        requirementLabel={t.commandLineup}
        requirementValue={scenarioCopy.label}
        requirementMeta={scenarioCopy.meta}
      />

      <ScoutingRoleSwitch
        label={t.filterSubrole}
        items={SUBROLE_ORDER.map(item => ({
          value: item,
          label: getSubroleLabel(item, locale),
          secondary: locale === 'en-US' ? '' : getSubroleLabel(item, 'en-US')
        }))}
        activeValue={subrole}
      ...
      <div className={styles.roleCockpitLeadGrid}>
        <ScoutingRoleLeader
          artwork={<HeroArtwork hero={primaryHero} role={leader.role} className={styles.roleCockpitArtwork} loading="eager" />}
          marketStatus={structure.status}
          verdict={`${t.marketVerdict} · ${getMarketStructureLabel(structure.status, t)}`}
          name={leader.identity.displayName}
          meta={`${leader.identity.teamShort} · ${getCandidateStatusLabel(leader, locale)} · #${leader.highSampleSubroleRank}/${leader.highSampleSubroleTotal}`}
          score={leaderFit?.score ?? '—'}
          scoreLabel={`${t.scenarioFitScore} · /100`}
          summary={getRoleFocusRead(players, structure, scenario, locale)}
          stats={{ density: audience, items: leadStats.map(([label, value]) => ({ label, value })) }}
        />

        <ScoutingRoleFocusPanel label={t.commandLineup} value={scenarioCopy.label} meta={scenarioCopy.meta} footerLabel="MODEL WEIGHTS" footerText={scenarioCopy.formula}>
          <div>
            {RECRUITMENT_SCENARIO_ORDER.map((item, index) => {
              const copy = getRecruitmentScenarioCopy(item, locale)
              return (
...
function ScoutingHeader({ accessRecord, locale, onLocaleChange }) {
  const t = getCopy(locale)
  return <SharedScoutingReportHeader
    brandTo="https://fries-cup.com/"
    accessLabel={t.access}
    accessValue={accessRecord.label}
    languageLabel="Language"
    languages={REVIEW_LOCALES.map(item => ({ value: item.id, label: item.param.toUpperCase() }))}
...
function OverviewHero({ locale }) {
  const t = getCopy(locale)
  return <SharedScoutingReportHero
    eyebrow={t.confidential}
    title={t.title}
    titleLead={t.titleLead}
    titleReport={t.titleReport}
    subtitle={t.subtitle}
    scopeLabel={t.sampleGate}
    scopeValue={t.sampleGateValue}
    scopeMeta="FCR 2026 · ROLE-RELATIVE"
  />
}
...
function ReportVersionStamp({ meta, locale }) {
  const t = getCopy(locale)
  return <SharedScoutingReportVersion items={[
    { label: t.reportVersion, value: meta?.reportVersion || 'FCR26 Scouting v2.3' },
    { label: t.dataAsOf, value: formatDataAsOf(meta?.dataAsOf, locale) }
  ]} />
}

function AudienceModeSwitch({ locale, audience, onChange }) {
  const t = getCopy(locale)
  return <SharedScoutingReportAudience
    label={t.audienceMode}
    activeValue={audience}
    activeLabel={audience === 'coach' ? t.coachView : t.managerView}
    activeHint={audience === 'coach' ? t.coachViewMeta : t.managerViewMeta}
    options={[{ value: 'manager', label: t.managerView }, { value: 'coach', label: t.coachView }]}
    onChange={onChange}
  />
}
