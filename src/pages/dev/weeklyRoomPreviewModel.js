...
export const PREVIEW_STAGES = { opening: '先手权', choosing: '选图', lineup: '首发', banorder: 'Ban 顺序', banning: 'Ban', ready: '准备', live: '比赛中', paused: '暂停', review: '图结果', result: '整场赛果' }
...
export function buildWeeklyRoomPreview(stage = 'lineup', role = 'representative') {
  ...
  const complete = !['opening', 'choosing', 'lineup', 'banorder', 'banning'].includes(stage)
  ...
  const opening = {
    ...
    access: { playTeams: [], canSetOneVOne: staff, canReportOneVOne: staff, canChoose: staff || role === 'representative', canChooseBanOrder: stage === 'banorder' && (staff || role === 'representative'), canBan: stage === 'banning' && (staff || role === 'representative'), canNext: staff },
