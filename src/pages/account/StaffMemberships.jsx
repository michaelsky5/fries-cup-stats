...
export default function StaffMemberships() {
  ...
  useEffect(() => {
    ...
    platformRequest('/me/staff-memberships', { signal: controller.signal })
      .then(result => { if (!controller.signal.aborted) setData(result.memberships) })
  ...
  return <section className={staffStyles.panel} aria-label={t("工作人员身份")}>
    <h3>{t("工作人员身份")}</h3>
    {error ? <><p role="alert">{t(error)}</p><button className={styles.secondary} onClick={() => setAttempt(value => value + 1)}>{t("重新读取身份")}</button></> : !data ? <p role="status">{t("正在读取本届工作人员关系…")}</p> : !data.length ? <p>{t("赛管与解说由赛事管理员邀请。已有选手账号直接认领邀请，无需另建账号。")}</p> : <ul className={staffStyles.events}>{events.map(event => <li key={event.id}>
