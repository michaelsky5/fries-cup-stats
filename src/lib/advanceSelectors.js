...
function normalizeGroupStandingRow(raw, groupLabel, index, teamsById, rules, groupComplete, favorites) {
  ...
  const qualified = Boolean(raw?.qualified) || (groupComplete && !requiresTiebreak && rank <= rules.advancePerGroup)
  const team = {
    ...sourceTeam,
    ...raw,
...
export function getSwissOverview(db, season) {
  const swissMatches = getSwissMatches(db)
  const officialMatches = swissMatches.filter(match => !isByeTeam(match?.team_a) && !isByeTeam(match?.team_b))
  const completed = officialMatches.filter(isComplete)
  const live = officialMatches.filter(isLive)
  const upcoming = officialMatches.filter(match => !isComplete(match) && !isLive(match))
  const completedRecords = swissMatches.filter(isComplete)
  const rules = getSwissRules(season, db)
  const currentMatch = officialMatches.find(match => isLive(match) || !isComplete(match)) ||
  ...
  const roundMatches = currentRoundKey
    ? officialMatches.filter(match => getRoundKey(match?.round || match?.stage) === currentRoundKey)
    : officialMatches
  ...
  return {
    ...
    completedMatches: completed.length,
    totalRecords: swissMatches.length,
    completedRecords: completedRecords.length,
    byeRecords: swissMatches.length - officialMatches.length,
...
export function getFinalRanking(db, { seasonFinished = false } = {}) {
  const teams = safeArr(db?.teams)
  const publishedRanking = teams
    .filter(team => toNumber(team?.final_rank, 0) > 0)
    .sort((a, b) => toNumber(a.final_rank, 999) - toNumber(b.final_rank, 999))

  if (publishedRanking.length || !seasonFinished || !teams.length) return publishedRanking

  const currentRanking = teams
    .map(team => ({ team, rank: toNumber(team?.current_rank, 0) }))
    .filter(item => item.rank > 0)
    .sort((a, b) => a.rank - b.rank)
  const isCompletePublishedOrder = currentRanking.length === teams.length &&
    currentRanking.every((item, index) => item.rank === index + 1)

  if (!isCompletePublishedOrder) return []

  return currentRanking.map(({ team, rank }) => ({
    ...team,
    final_rank: rank,
    final_rank_source: 'current_rank'
  }))
}
...
export function getChampion(db, options) {
  const ranked = getFinalRanking(db, options)
...
export function getFinalResult(db, options) {
  const champion = getChampion(db, options)
  ...
  return {
    ...
    finalRanking: getFinalRanking(db, options),
...
export function getAdvanceSummary(db, season) {
  const phaseState = getAdvancePhaseState(db, season)
  const finalResult = getFinalResult(db, { seasonFinished: phaseState.seasonFinished })
  const defaultPhase = getDefaultAdvancePhase(db, season)
  const phases = getAdvancePhases(season, db)
