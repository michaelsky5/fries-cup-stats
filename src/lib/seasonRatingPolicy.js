...
export function getSeasonRatingStatusLabel(entry, locale = 'zh-CN') {
  ...
  if (status === 'FORMAL') return en ? 'Ranked' : '正式排名'
  if (status === 'PROVISIONAL') return en ? 'Provisional' : '暂定评分'
...
export function getSeasonRatingLabel(entry, locale = 'zh-CN') {
  return entry?.seasonRatingStatus === 'PROVISIONAL'
...
export function formatSeasonSampleRequirements(minTimeMins = 30, locale = 'zh-CN') {
  ...
  return locale === 'en-US'
    ...
    : `本职责至少 ${rule.maps} 图 · ${rule.minutes} 分钟 · ${rule.matches} 场比赛`
}
