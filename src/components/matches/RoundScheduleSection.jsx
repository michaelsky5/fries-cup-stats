...
export default function RoundScheduleSection({ hub }) {
  ...
  const isGroupDay = String(hub?.currentRoundSummary?.stage || '').toUpperCase() === 'GROUP'
  const defaultPanel = getDefaultPanel(hasUpcoming, hasResults)
  const [activePanel, setActivePanel] = useState(defaultPanel)
  const sectionTitle = !hasUpcoming && hasResults ? '最近赛果' : isGroupDay ? '本比赛日赛程' : '本轮赛程'
  ...
  return (
    <section className={styles.section} aria-labelledby="round-schedule-title" data-testid="round-schedule-section">
      ...
      {showTabs ? (
        <div className={styles.statusTabs} role="tablist" aria-label={uiText("比赛状态", uiLocale)} data-testid="match-status-tabs">
          {hasUpcoming ? (
            <button
              type="button"
              role="tab"
      ...
      <p className={styles.scheduleNote}>{uiText("赛程时间以最新公布信息为准。赛果与选手数据将在赛后核对完成后更新。", uiLocale)}</p>
    </section>
  )
}
