...
export default function SignalMatchIndex({ rows, groups, filters, options, activeTab, favoriteCount, updateQuery, resetFilters, setTab, focusSearch, hub, stageEntries }) {
  ...
  return <div className={styles.shell} data-page-mode="index" data-match-design="signal-index" data-i18n-ignore>
    ...
    {hub.isArchive ? <nav className={styles.stages} style={{ '--stage-count': stageOrder.length }} data-paired={en && stageOrder.length > 3 || undefined} aria-label={en ? 'Match stages' : uiText("比赛阶段", locale)}>{stageOrder.map(entry => <button key={entry.value} type="button" aria-pressed={filters.stage === entry.value} onClick={() => selectStage(entry.value)}><strong>{entry.value === 'ALL' ? (en ? 'All stages' : uiText("全部阶段", locale)) : getScheduleStageLabel(entry.value, locale)}</strong><span>{entry.count}</span></button>)}</nav> : null}
    <div className={styles.filterBar}>
      ...
      <button type="button" className={styles.filterButton} aria-expanded={advancedOpen} aria-controls="match-index-filters" onClick={() => setAdvancedOpen(open => !open)}><span>{en ? 'More filters' : uiText("更多筛选", locale)}</span><b data-active={dirtyCount > 0}>{dirtyCount || (advancedOpen ? '−' : '＋')}</b></button>
    </div>
