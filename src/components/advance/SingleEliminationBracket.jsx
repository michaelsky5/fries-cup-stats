    ...
    <section className={`${styles.singleElimColumn} ${styles.singleElimMedalColumn}`}>
      <header className={styles.singleElimColumnHeader}>
        ...
        <div>
          ...
          <strong>{copy(locale, uiText("决赛日", locale), 'Finals Day')}</strong>
        </div>
    ...
    <div className={styles.phaseStack}>
      <section className={styles.singleElimDashboard}>
        <AdvancePhaseHero
          ...
          description={copy(
            locale,
            uiText("八强单败晋级 · 八强赛与半决赛 FT3 · 季军赛与总决赛 FT4", locale),
            'Single-elimination top eight · quarterfinals and semifinals FT3 · medal matches FT4'
          )}
          metrics={[
            { value: teamCount || 8, label: copy(locale, uiText("支晋级队伍", locale), 'Qualified Teams') },
            { value: allMatches.length, label: copy(locale, uiText("场淘汰赛", locale), 'Playoff Matches') },
            { value: 1, label: copy(locale, uiText("个冠军席位", locale), 'Champion Slot'), accent: true }
          ]}
        />
        <div className={styles.singleElimSummary}>
          <div><span>{copy(locale, uiText("淘汰赛进度", locale), 'Playoff Progress')}</span><strong>{completedMatches} / {allMatches.length}</strong></div>
          <div><span>{copy(locale, uiText("当前轮次", locale), 'Current Round')}</span><strong>{getRoundTitle(currentRound, locale)}</strong></div>
          <div><span>{copy(locale, uiText("八强赛 / 半决赛", locale), 'Quarterfinals / Semifinals')}</span><strong>FT3</strong></div>
          <div><span>{copy(locale, uiText("季军赛 / 总决赛", locale), 'Third Place / Grand Finals')}</span><strong>FT4</strong></div>
        </div>
      </section>
      ...
      <section className={styles.singleElimPath} aria-labelledby="single-elim-path-title">
        <header>
          ...
          <h2 id="single-elim-path-title">{copy(locale, uiText("晋级路径", locale), 'Advancement Path')}</h2>
        </header>
        <ol>
          {rounds.map((round, index) => (
            <li key={round.id}>
              <b>{String(index + 1).padStart(2, '0')}</b>
              <span><strong>{getRoundTitle(round, locale)}</strong><em>{round.matches.length} {copy(locale, uiText("场", locale), 'matches')} · {index < 2 ? 'FT3' : 'FT4'}</em></span>
            </li>
          ))}
        </ol>
      </section>
      ...
      <section className={styles.singleElimBoard}>
        <header className={styles.sectionHeader}>
          <div>
            ...
            <h2>{copy(locale, uiText("从八强到冠军", locale), 'Road to the Championship')}</h2>
          </div>
          <p>{copy(locale, uiText("对阵按真实晋级顺序排列；决赛日集中展示冠军与季军归属。", locale), 'Matches follow the real advancement order, with both medal matches grouped on finals day.')}</p>
        </header>
        <div className={styles.singleElimScrollHint}>{copy(locale, uiText("晋级路线  4 场 → 2 场 → 决赛日", locale), 'Bracket route  4 matches → 2 matches → finals day')}</div>
