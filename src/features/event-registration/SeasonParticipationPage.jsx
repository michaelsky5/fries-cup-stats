...
function Workspace({ user, seasonId }) {
  ...
  return <>
    ...
    {workspace.registrations.map(record => <Registration key={`${record.id}:${record.revision}`} record={record} userId={user.id} owner={record.ownerUserId === user.id} canWrite={workspace.canWrite} busy={busy} perform={perform} invitationLinks={invitationLinks} onRefresh={async () => { if (await confirmDiscard()) setVersion(value => value + 1) }} />)}
...
function TeamForm({ organizations = [], record, busy, onSave }) {
  ...
  return <section className={styles.panel}><h2>{record ? uiText("修改报名资料", uiLocale) : uiText("创建队伍报名", uiLocale)}</h2><form onSubmit={async event => { event.preventDefault(); const payload = { ...form, name: form.name.trim(), shortName: form.shortName.trim(), contact: form.contact.trim(), note: form.note.trim(), ...(form.organizationId ? {} : !record ? { organizationId: undefined } : {}), ...(record ? { revision: record.revision } : {}) }; if (await onSave(payload)) setDirty(false) }}>
...
function Registration({ record, userId, owner, canWrite, busy, perform, invitationLinks, onRefresh }) {
  const uiLocale = useUiLocale()
  ...
  const rosterFull = record.members.length >= 7
  ...
  return <section className={styles.panel}>
    ...
    {owner && editable && confirmed && <p className={styles.feedback} role="status">{uiText("全部队员已确认，下一步提交这份队伍报名。", uiLocale)}</p>}
    <div className={styles.tableScroll}><table><thead><tr><th>{uiText("选手", uiLocale)}</th><th>BattleTag</th><th>{uiText("职责", uiLocale)}</th><th>{uiText("本人确认", uiLocale)}</th>{editable && <th>{uiText("操作", uiLocale)}</th>}</tr></thead><tbody>{record.members.map(member => <tr key={member.id}><td>{member.displayName}</td><td>{member.battleTag}</td><td>{roleLabels[member.role]}</td><td>{statusLabels[member.status]}</td>{editable && <td><div className={styles.memberActions}>{member.status === 'INVITED' ? <button type="button" disabled={busy} onClick={() => perform(`${prefix}/invitations`, { revision: record.revision, email: member.email, displayName: member.displayName, battleTag: member.battleTag, role: member.role }, 'POST', '选手邀请')}>{uiText("重新生成链接", uiLocale)}</button> : null}{invitationLinks?.[member.id] ? <button type="button" disabled={busy} onClick={() => navigator.clipboard?.writeText(invitationLinks[member.id].activationUrl)}>{uiText("复制链接", uiLocale)}</button> : null}{member.status === 'DRAFT' || member.status === 'INVITED' ? <button type="button" disabled={busy} onClick={() => { if (window.confirm(uiText("确定从这份报名草稿中移出 {0} 吗？", uiLocale, [member.displayName]))) perform(`${prefix}/remove-member`, { revision: record.revision, memberId: member.id }) }}>{uiText("移出草稿", uiLocale)}</button> : null}</div></td>}</tr>)}</tbody></table></div>
    {owner && editable && record.members.some(member => member.status === 'CONFIRMED') ? <p className={styles.feedback} role="status">{uiText("已确认的选手资料不能直接修改。如需纠正，请让本人先撤回确认；状态变为待确认后，再移出成员并重新邀请。", uiLocale)}</p> : null}
    ...
    {editable && <>
      <form className={styles.playerForm} onSubmit={async event => { event.preventDefault(); const payload = { ...player, displayName: player.displayName.trim(), email: player.email.trim(), battleTag: player.battleTag.trim(), revision: record.revision }; if (await perform(`${prefix}/invitations`, payload, 'POST', '选手邀请')) setPlayer(emptyPlayer) }}>
        ...
        </div>{rosterFull ? <p className={styles.warning} role="status">{uiText("当前名单已达到 7 人上限。请先移除一名草稿成员，再邀请其他选手。", uiLocale)}</p> : null}<button disabled={busy || rosterFull}>{rosterFull ? uiText("名单已满", uiLocale) : uiText("生成本人确认邀请", uiLocale)}</button>
      </form>
