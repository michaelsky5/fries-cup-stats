  ...
  return <div className={styles.page} data-weekly-overview="true">
    ...
    <div className={styles.mainGrid}>
          ...
          {focus ? <div className={styles.spotlight} id="weekly-focus" ref={focusRef} data-match-state={focus.state}>
            ...
            <div className={styles.scoreboard}>
              {[focus.team_a, focus.team_b].map((team, side) => <div className={side ? styles.teamB : styles.teamA} key={side}>
                ...
                <div className={styles.teamIdentity}><h3>{shortName(team)}</h3><p>{fullName(team)}</p>
                <button type="button" className={styles.follow} aria-label={`${t(uiText('关注队伍', locale), 'Follow team')} ${shortName(team)}`} aria-pressed={!!isFavoriteTeam?.(idOf(team))} onClick={() => toggleTeamFavorite(idOf(team))}>{isFavoriteTeam?.(idOf(team)) ? '★' : '☆'} <span>{t(isFavoriteTeam?.(idOf(team)) ? uiText("已关注", locale) : uiText("关注队伍", locale), isFavoriteTeam?.(idOf(team)) ? 'Following' : 'Follow team')}</span></button>
                </div>
              </div>)}
      ...
      <aside className={styles.standings} aria-labelledby="weekly-standings-title">
        ...
        <div className={styles.standingsIntro}>{isPilot && <p>{t(uiText("比赛照常记录。", locale), 'Every match is still recorded.')}</p>}<span>{isPilot ? t(uiText("本周期不计入正式积分。", locale), 'This cycle does not award standings points.') : t(uiText("展示本周期最新已公布积分，不随周次回溯。", locale), 'Latest published cycle totals, across all weeks.')}</span></div>
        {!isPilot && standings.length > 0 ? <>
          <table><thead><tr><th scope="col">{t(uiText("名次", locale), 'Rank')}</th><th scope="col">{t(uiText("队伍", locale), 'Team')}</th><th scope="col">{t(uiText("已赛", locale), 'Played')}</th><th scope="col">{t(uiText("积分", locale), 'Pts')}</th></tr></thead><tbody>{standings.map(row => <tr key={row.team_id} data-focused={focusedIds.includes(row.team_id) || undefined} data-unplayed={row.played === 0 || undefined}><td>{row.display_rank ?? '—'}{row.tied && <small>{t(uiText("并列", locale), 'Tied')}</small>}</td><th scope="row"><Link to={withSeason(`/teams/${encodeURIComponent(row.team_id)}`)} {...returnProps}><TeamLogo team={row.team} seasonId={seasonId} className={styles.rowLogo} />{row.team_short_name || row.team_name}</Link></th><td>{row.played ?? '—'}</td><td><strong>{row.points ?? '—'}</strong></td></tr>)}</tbody></table>
      ...
      <section className={styles.followingSection} id="weekly-following">
        ...
        <div className={styles.followTeams}>{(db?.teams || []).map(team => <button type="button" key={idOf(team)} aria-label={`${t(uiText('关注队伍', locale), 'Follow team')} ${shortName(team)}`} aria-pressed={!!isFavoriteTeam?.(idOf(team))} onClick={() => toggleTeamFavorite(idOf(team))}><TeamLogo team={team} seasonId={seasonId} className={styles.rowLogo} /><b>{shortName(team)}</b><span aria-hidden="true">{isFavoriteTeam?.(idOf(team)) ? '★' : '☆'}</span></button>)}</div>
      </section>
    </div>
