              ...
              <div className={styles.layout}>
                ...
                {room ? (
                  <article ref={roomCardRef} tabIndex={-1} className={styles.roomCard} aria-label={uiText("{0} 对阵 {1}", uiLocale, [weeklyTeamName(room.teamA), weeklyTeamName(room.teamB)])}>
                    ...
                    {journey.preMatch && !room.myTeams?.length && <p className={styles.roomNotice}>{uiText("进入比赛房后，按本场身份完成赛前准备、选图、首发和禁用。", uiLocale)}</p>}
                    {journey.preMatch ? !!room.myTeams?.length && <details className={styles.coordinationDetails}><summary>{uiText("房间安排与赛管协助", uiLocale)}</summary>{room.myTeams.map(ownTeam => <WeeklyCoordinationPanel key={`${room.id}:${ownTeam.team.id}`} weekId={room.week.id} teamId={ownTeam.team.id} matchId={room.id} readOnly={Boolean(loadError) || currentWorkspace?.featureAccess !== 'WRITE'} onActivityChange={onActivityChange} />)}</details> : <div className={styles.responses}>
