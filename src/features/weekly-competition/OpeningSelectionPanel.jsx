...
function OpeningForm({ data, disabled, mutate }) {
  ...
  const rightsLabel = retained ? '平局 · 保留选择方' : later ? '上一图败方选择' : firstPickMode === 'HIGH_SEED' ? '高种子优先' : firstPickMode === 'MANUAL' ? opening.firstPick?.decision?.confirmation === 'BOTH_TEAMS' ? '双方已确认' : '赛管已确认' : '对决获胜'
  ...
  const needsAttackDefense = ['Hybrid', 'Escort'].includes(chosenMap?.type)
  ...
  return <section className={`${styles.opening} ${surfaces.paper}`} aria-label={uiText("开局与选禁", uiLocale)} data-room-slot="opening" data-phase={opening.phase}>
    ...
    {!later && !['CHOOSING', 'BANNING', 'CONFIRMING_FIRST_PICK'].includes(opening.phase) && <OpeningMethodControl key={opening.revision} opening={opening} disabled={disabled || !!draft.pending} send={send} />}
    ...
    {firstPickMode === 'MANUAL' && ['NOT_STARTED', 'CONFIRMING_FIRST_PICK'].includes(opening.phase) && <FirstPickConfirmation data={data} disabled={disabled || !!draft.pending} send={send} mutate={mutate} />}
    {opening.phase === 'CHOOSING' && <><p className={styles.rule}>{retained ? uiText("上一图平局，保留选择方 ", uiLocale) : later ? uiText("上一图败方 ", uiLocale) : ''}{teamName(winnerTeam)}{uiText(" 选择第 ", uiLocale)}{opening.mapOrder}{uiText(" 图；有攻防的模式同时确认本队先攻或先防。", uiLocale)}</p>{access.canChoose ? <>
      {later ? <><div className={styles.typeHeading}><strong>{uiText("地图类型 · 第 ", uiLocale)}{opening.typeCycle?.round || 1}{uiText(" 轮", uiLocale)}</strong><small>{opening.typeCycle?.usedCount || 0}{uiText(" / 5 已用 · 五种选完后重开", uiLocale)}</small></div>
      <div className={styles.types} role="group" aria-label={uiText("选择地图类型", uiLocale)}>{opening.typeCycle?.types.map(item => <button key={item.type} aria-pressed={mapType === item.type} disabled={disabled || !!draft.pending || item.used || !item.remaining} onClick={() => edit({ mapType: item.type, mapName: '', startSide: '' })}><strong>{mapTypeName(item.type)}</strong><small>{item.used ? uiText("本轮已用", uiLocale) : item.remaining ? uiText("{0} 张可选", uiLocale, [item.remaining]) : uiText("地图已用尽", uiLocale)}</small></button>)}</div></> : <div className={styles.typeHeading}><strong>{uiText("图一固定地图类型 · 占领要点", uiLocale)}</strong><small>{uiText("选择权方只选择具体地图", uiLocale)}</small></div>}
      <div className={styles.mapPool} role="group" aria-label={uiText("选择第 {0} 图", uiLocale, [opening.mapOrder])}>{opening.rules.maps.filter(map => map.type === mapType).map(map => <button key={map.name} aria-pressed={draft.mapName === map.name} disabled={disabled || !!draft.pending || !!map.reason} title={map.reason || formatOwMapName(map.name, uiLocale)} onClick={() => edit({ mapName: map.name })} style={getOwMap(map.name) ? { backgroundImage: `linear-gradient(0deg,#181a17ed,#181a1710),url("${getMapImage(map.type, map.name)}")` } : undefined}>{draft.mapName === map.name && <span className={styles.mapSelected} aria-hidden="true">{uiText("✓ 已选", uiLocale)}</span>}<strong>{formatOwMapName(map.name, uiLocale)}</strong><small>{map.reason || mapTypeName(map.type)}</small></button>)}</div>
      {!mapType && <p className={styles.warning}>{uiText("本轮剩余类型没有可用地图，请赛管联系管理员补充地图池。", uiLocale)}</p>}
      {needsAttackDefense && <div className={styles.order} role="group" aria-label={uiText("选择本队攻防顺序", uiLocale)}>{[['ATTACK', '本队先攻', '对方先防'], ['DEFEND', '本队先防', '对方先攻']].map(([side, label, other]) => <button key={side} aria-pressed={draft.startSide === side} disabled={disabled || !!draft.pending} onClick={() => edit({ startSide: side })}>{uiText(label, uiLocale)}<small>{uiText(other, uiLocale)}</small></button>)}</div>}
      <div className={styles.confirm}><small>{chosenMap ? formatOwMapName(chosenMap.name, uiLocale) : uiText("请选择地图", uiLocale)}{needsAttackDefense ? ' · ' + uiText(draft.startSide === 'ATTACK' ? '本队先攻' : draft.startSide === 'DEFEND' ? '本队先防' : '请选择攻防顺序', uiLocale) : ''}</small><button className={styles.primary} disabled={disabled || !!draft.pending || !chosenMap || needsAttackDefense && !draft.startSide} onClick={() => send('SELECT_SETUP', { teamId: winnerTeam.id, mapName: chosenMap.name, mapType: chosenMap.type, ...(needsAttackDefense ? { startSide: draft.startSide } : {}) })}>{uiText("确认地图，进入首发确认", uiLocale)}</button></div>
    </> : <div className={styles.result}><strong>{teamName(winnerTeam)} · {rightsLabel}</strong>{!later && <span>{round?.sides.map(side => `${teamName(team(side.side))} ${handName(side.move)}`).join(' / ')}</span>}<p>{uiText("等待 ", uiLocale)}{representativeName(winnerTeam)}{uiText(" 确认地图及适用的攻防顺序，随后双方提交首发。", uiLocale)}</p></div>}</>}
    {choosingBanOrder && <>
      ...
      <div className={styles.confirm}><small>{uiText("确认顺序后，先手方开始禁用英雄。", uiLocale)}</small><button className={styles.primary} disabled={disabled || !!draft.pending || !draft.banOrder} onClick={() => send('SELECT_BAN_ORDER', { teamId: winnerTeam.id, banOrder: draft.banOrder })}>{uiText("确认 Ban 顺序", uiLocale)}</button></div></> : <p className={styles.waiting} role="status">{uiText("等待 {0} 确认 Ban 顺序。", uiLocale, [representativeName(winnerTeam)])}</p>}
    </>}
    {opening.phase === 'BANNING' && !choosingBanOrder && <><div className={styles.banSummary}><strong>{formatOwMapName(opening.setup?.name, uiLocale)}</strong><span>{teamName(team(opening.setup?.firstBanSide))}{uiText(" 先 Ban", uiLocale)}</span><span>{teamName(data.match.teamA)}：{formatOwHeroName(opening.setup?.banA, uiLocale) || uiText("待选择", uiLocale)}</span><span>{teamName(data.match.teamB)}：{formatOwHeroName(opening.setup?.banB, uiLocale) || uiText("待选择", uiLocale)}</span></div>
      {access.canBan ? <><div className={styles.filters} role="group" aria-label={uiText("筛选英雄职责", uiLocale)}>{[['TANK', '重装'], ['DPS', '输出'], ['SUP', '支援']].map(([key, label]) => <button key={key} aria-pressed={role === key} onClick={() => setRole(key)}>{uiText(label, uiLocale)}</button>)}<small>{opening.rules.differentRolesPerMap ? uiText("双方禁用不同职责", uiLocale) : uiText("双方各禁用 1 名英雄", uiLocale)}</small></div><div className={styles.heroes} role="group" aria-label={uiText("选择禁用英雄", uiLocale)}>{opening.heroes.filter(hero => role === 'ALL' || hero.role === role).map(hero => <button key={hero.name} title={hero.reason || formatOwHeroName(hero.name, uiLocale)} aria-pressed={draft.hero === hero.name} disabled={disabled || !!hero.reason || !!draft.pending} onClick={() => edit({ hero: hero.name })}>{getOwHero(hero.name) && <img src={getHeroImage(hero.name, hero.role)} alt="" onError={e => { e.currentTarget.hidden = true }} />}<span>{formatOwHeroName(hero.name, uiLocale)}</span>{hero.reason && <small>{hero.reason}</small>}</button>)}</div><div className={styles.confirm}><small>{chosenHero ? uiText("禁用 {0} · 确认后锁定", uiLocale, [formatOwHeroName(chosenHero.name, uiLocale)]) : opening.setup?.banA || opening.setup?.banB ? uiText("选择一名英雄，确认后等待实际开赛", uiLocale) : uiText("选择一名英雄，确认后轮到对方", uiLocale)}</small><button className={styles.primary} disabled={disabled || !chosenHero} onClick={() => send('BAN', { teamId: nextTeam.id, hero: chosenHero.name, heroRole: chosenHero.role })}>{uiText("确认 Ban", uiLocale)}{chosenHero ? ` ${formatOwHeroName(chosenHero.name, uiLocale)}` : ''}</button></div></> : <p className={styles.waiting} role="status">{uiText("等待 ", uiLocale)}{representativeName(nextTeam)}{uiText(" 完成禁用。你可以继续查看名单和比赛沟通。", uiLocale)}</p>}
    </>}
  </section>
}
...
export function OpeningHistory({ data, disabled, mutate }) {
  ...
  return <details className={styles.history}><summary>{uiText("选禁记录 · 第 ", uiLocale)}{opening.mapOrder}{uiText(" 图 ", uiLocale)}{opening.winner ? uiText("{0} 决定地图与 Ban 先后", uiLocale, [team(opening.winner)]) : uiText("首图选择权待确定", uiLocale)}</summary>
    ...
    {(opening.selections || []).map(item => <p key={item.mapOrder}>{uiText("第 ", uiLocale)}{item.mapOrder}{uiText(" 图 · ", uiLocale)}{formatOwMapName(item.mapName, uiLocale)} · {team(item.chooserSide)}{uiText(" 选图 / ", uiLocale)}{team(item.firstBanSide)}{uiText(" 先 Ban · ", uiLocale)}{formatOwHeroName(item.banA, uiLocale)} / {formatOwHeroName(item.banB, uiLocale)}</p>)}
    {opening.setup && <p>{formatOwMapName(opening.setup.name, uiLocale)} · {team(opening.setup.firstBanSide)}{uiText(" 先 Ban · ", uiLocale)}{team('A')}{uiText(" 禁用 ", uiLocale)}{formatOwHeroName(opening.setup.banA, uiLocale) || uiText("待定", uiLocale)} / {team('B')}{uiText(" 禁用 ", uiLocale)}{formatOwHeroName(opening.setup.banB, uiLocale) || uiText("待定", uiLocale)}</p>}
