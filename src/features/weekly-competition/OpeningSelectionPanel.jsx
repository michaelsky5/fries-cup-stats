...
function OpeningForm({ data, disabled, mutate }) {
  ...
  const [draft, setDraft] = useState(() => loadDraft(key)), [selectedRole, setRole] = useState(''), [storageError, setStorageError] = useState('')
  ...
  return <section className={`${styles.opening} ${surfaces.paper}`} aria-label={uiText("开局与选禁", uiLocale)} data-room-slot="opening" data-phase={opening.phase}>
    ...
    {opening.phase === 'CHOOSING' && <><p className={styles.rule}>{retained ? uiText("上一图平局，保留选择方 ", uiLocale) : later ? uiText("上一图败方 ", uiLocale) : ''}{teamName(winnerTeam)}{uiText(" 选择第 ", uiLocale)}{opening.mapOrder}{uiText(" 图；有攻防的模式同时确认本队先攻或先防。", uiLocale)}</p>{access.canChoose ? <>
      ...
      <div className={styles.confirm}><small>{chosenMap ? formatOwMapName(chosenMap.name) : uiText("请选择地图", uiLocale)}{needsAttackDefense ? ' · ' + uiText(draft.startSide === 'ATTACK' ? '本队先攻' : draft.startSide === 'DEFEND' ? '本队先防' : '请选择攻防顺序', uiLocale) : ''}</small><button className={styles.primary} disabled={disabled || !!draft.pending || !chosenMap || needsAttackDefense && !draft.startSide} onClick={() => send('SELECT_SETUP', { teamId: winnerTeam.id, mapName: chosenMap.name, mapType: chosenMap.type, ...(needsAttackDefense ? { startSide: draft.startSide } : {}) })}>{uiText("确认地图，进入首发确认", uiLocale)}</button></div>
