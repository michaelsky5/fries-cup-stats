...
export default function RoomRefereeAssignments({ data, disabled, mutate }) {
  ...
  const canManage = data.access.administrator && !data.simulation
  ...
  return <details className={styles.staffDetails}>
    ...
    {referees.length ? <ul className={styles.casterList}>{referees.map(person => <li key={person.id || person.displayName}>
      <div><strong>{person.displayName}</strong><span>{person.source === 'BROADCAST_PLAN' ? t("已发布排班") : t("本场手动指派")}</span></div>
      {canManage && <button type="button" disabled={disabled} onClick={() => update(person, false)}>{t("撤回 ")}{person.displayName}{t(" 的本场赛管权限")}</button>}
    ...
    {canManage && <>
      {!!overrides.length && <div className={styles.casterOverrides}><strong>{t("本场手动设置")}</strong><ul className={styles.casterList}>{overrides.map(person => <li key={person.id}>
        ...
        <button type="button" disabled={disabled} onClick={() => update(person, false, true)}>{t("恢复 ")}{person.displayName}{t(" 跟随排班")}</button>
      </li>)}</ul></div>}
