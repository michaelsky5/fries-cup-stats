...
export function RoomStageRail({ data, selectedStage, onStageSelect }) {
  ...
  const selectedSetup = data.opening?.setup || (map?.name ? { name: map.name, type: map.type, chooserSide: map.chooserSide, firstBanSide: map.firstBanSide, banA: map.banA, banB: map.banB } : null)
  const chooser = selectedSetup?.chooserSide === 'A' ? data.match.teamA : selectedSetup?.chooserSide === 'B' ? data.match.teamB : null
  const firstBan = selectedSetup?.firstBanSide === 'A' ? data.match.teamA : selectedSetup?.firstBanSide === 'B' ? data.match.teamB : null
  ...
  const mapSelectionPage = view === 1 && view !== current ? <section className={styles.mapSelectionPage} data-room-slot="stage-record" aria-label={uiText("地图选择", uiLocale)}>
    ...
    <div className={styles.mapSelectionFacts}><span><b>{uiText("选图方", uiLocale)}</b>{chooser?.shortName || chooser?.name || uiText("待定", uiLocale)}</span><span><b>{uiText("先 Ban", uiLocale)}</b>{firstBan?.shortName || firstBan?.name || uiText("待定", uiLocale)}</span><span><b>{uiText("当前地图", uiLocale)}</b>{formatOwMapName(selectedSetup?.name) || uiText("待定", uiLocale)}</span></div>
  ...
  const readOnlySummary = view !== current ? view === 1 ? <><strong>{formatOwMapName(selectedSetup?.name) || uiText("地图尚未确定", uiLocale)}</strong><span>{selectedSetup?.type || uiText("地图类型待定", uiLocale)} · {uiText("选图方", uiLocale)} {chooser?.shortName || chooser?.name || uiText("待定", uiLocale)} · {uiText("先 Ban", uiLocale)} {firstBan?.shortName || firstBan?.name || uiText("待定", uiLocale)}<br />{data.match.teamA.shortName || data.match.teamA.name}：{formatOwHeroName(selectedSetup?.banA) || uiText("待定", uiLocale)} · {data.match.teamB.shortName || data.match.teamB.name}：{formatOwHeroName(selectedSetup?.banB) || uiText("待定", uiLocale)}</span></> : view === 6 ? completedMaps || uiText("本图结果尚未记录", uiLocale) : view === 7 ? submissionUrl ? uiText("战报提交入口已生成，请点击第 08 步打开。", uiLocale) : uiText("整场地图完成后，由赛管生成 System 战报提交入口。", uiLocale) : `${labels[view]} · ${formatOwMapName(map?.name) || uiText("当前地图", uiLocale)} · ${uiText("只读查看，当前操作仍在第 {0} 步", uiLocale, [String(current + 1).padStart(2, '0')])}` : null
  return <>
