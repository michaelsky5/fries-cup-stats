...
export default function RoomRulesPanel({ data }) {
  ...
  return <details className={styles.roomRules}><summary>{uiText("本场地图池与规则 · ", uiLocale)}{rules.maps.length}{uiText(" 张 / 5 种类型", uiLocale)}</summary><div className={styles.ruleGrid}>{Object.entries(labels).map(([type, label]) => <div key={type}><strong>{label}</strong><ul>{rules.maps.filter(map => map.type === type).map(map => <li key={map.name}>{formatOwMapName(map.name)}<small>{data.maps.some(played => played.name === map.name) ? uiText("本场已选择", uiLocale) : uiText("地图池", uiLocale)}</small></li>)}</ul></div>)}</div><p>{uiText("图一固定为占领要点；实际游戏 1V1 胜者获得图一选择权。图二及后续由上一图败方选择地图类型、具体地图与适用的攻防；平局保留选择方。双方确认首发后，由选择方在英雄禁用环节决定本队先 Ban 或后 Ban。", uiLocale)}</p><p>{uiText("双方每图各 Ban 1 名英雄", uiLocale)}{rules.differentRolesPerMap ? uiText("，双方禁用职责不同", uiLocale) : ''}{rules.uniqueHeroesPerTeamPerSeries ? uiText("；同队整场不能重复禁用同一英雄", uiLocale) : ''}。</p></details>
}
