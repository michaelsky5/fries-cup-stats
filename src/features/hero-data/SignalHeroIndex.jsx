...
function RoleSpotlight({ role, heroes, samples, locale, isEn, toHero, index }) {
  ...
  return <article className={styles.spotlight} data-role={role}>
    <div className={styles.spotlightTop}><span>{roleLabel(role, isEn)}{!isEn ? <i>/ {role.toUpperCase()}</i> : null}</span><small>0{index + 1}</small></div>
    <Link to={toHero(leader.key)} className={styles.spotlightMain} aria-label={`${formatOwHeroName(leader.name, locale)} · ${isEn ? 'Explore hero' : uiText("查看英雄档案", locale)}`}>
    ...
    <div className={styles.runners}>{heroes.slice(1, 3).map(hero => <Link key={hero.key} to={toHero(hero.key)}><HeroPortrait name={hero.name} className={styles.runnerPortrait} /><span>{formatOwHeroName(hero.name, locale)}</span><b>{percentage(hero.rate)}</b><Arrow /></Link>)}</div>
  </article>
}
...
function HeroDirectory({ guide, params, setParams, locale, isEn }) {
  ...
  return <section className={styles.directory} id="hero-directory" aria-label={isEn ? 'Find and browse heroes' : uiText("英雄查找与浏览", locale)}>
    ...
    <header className={styles.directoryHeading}><div><h2 id="directory-heading">{search.trim() ? (isEn ? 'Matching heroes' : uiText("搜索结果", locale)) : role ? (isEn ? `${roleLabel(role, isEn)} heroes` : uiText("{0}英雄", locale, [roleLabel(role, isEn)])) : (isEn ? 'Hero directory' : uiText("英雄索引", locale))}</h2><span role="status" aria-live="polite">{visible.length} / {guide.heroes.length} {isEn ? 'recorded heroes' : uiText("位已收录英雄", locale)}</span></div>{filtered ? <button type="button" className={styles.resetFilters} onClick={() => update({ heroRole: '', heroSearch: '' })}>{isEn ? 'Clear filters' : uiText("清除筛选", locale)} ×</button> : <span>{isEn ? 'Select a hero to explore' : uiText("点选英雄，查看选手与比赛", locale)}<Arrow /></span>}</header>
    ...
    {visible.length ? <ol className={styles.heroGrid} data-sort={sort}>{visible.map(hero => <li key={hero.key}><Link to={heroGuideHref(params, hero.key)} className={styles.heroRow} data-role={hero.role}>
      <span className={styles.rowRank}>{sort === 'records' ? String(hero.rank).padStart(2, '0') : '↗'}</span><HeroPortrait name={hero.name} className={styles.portrait} /><span className={styles.rowIdentity}><strong>{formatOwHeroName(hero.name, locale)}</strong><small>{roleLabel(hero.role, isEn)}<i> / </i>{sort === 'players' ? `${hero.count} ${isEn ? 'appearances' : uiText("次出场", locale)}` : `${hero.players.length} ${isEn ? 'players' : uiText("位选手", locale)}`}</small></span><span className={styles.rowMetric}><b>{sort === 'players' ? hero.players.length : percentage(hero.rate)}</b><small>{sort === 'players' ? (isEn ? 'players' : uiText("位选手", locale)) : `${hero.count} ${isEn ? 'appearances' : uiText("次出场", locale)}`}</small></span><span className={styles.rowTrack} aria-hidden="true"><i style={{ width: percentage(hero.rate) }} /></span>
