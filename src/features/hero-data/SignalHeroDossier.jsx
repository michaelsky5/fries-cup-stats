...
function PlayerEvidence({ hero, isEn, withSeason, evidenceHref, expanded, onToggle }) {
  const uiLocale = useUiLocale()
  const location = useLocation()
  return <section className={styles.analysisPanel} id="hero-players" aria-labelledby="hero-players-heading">
    <header><h3 id="hero-players-heading">{isEn ? 'The players behind the hero' : uiText("谁把这个英雄带上场", uiLocale)}</h3><span>{hero.players.length} {isEn ? (hero.players.length === 1 ? 'player' : 'players') : uiText("位选手", uiLocale)}</span></header>
    ...
    {hero.players.length ? <ol className={styles.playerList}>{hero.players.slice(0, expanded ? undefined : 6).map((player, index) => <li key={player.key}><span className={styles.rowRank}>{String(index + 1).padStart(2, '0')}</span><div>{player.id ? <Link to={withSeason(`/players/${encodeURIComponent(player.id)}`)} state={getReturnState(location)} onClick={() => saveReturnScroll(location)}>{player.name}<Arrow /></Link> : <strong>{player.name}</strong>}<small>{player.teams.join(' / ') || '—'}</small></div><Link className={styles.evidenceCount} to={evidenceHref({ heroPlayer: player.key })} aria-label={isEn ? `View ${player.name}'s ${player.count} ${player.count === 1 ? 'appearance' : 'appearances'}` : uiText("查看 {0} 的 {1} 次出场记录", uiLocale, [player.name, player.count])}><b>{player.count}</b><Arrow /></Link></li>)}</ol> : <p className={styles.directoryNote}>{isEn ? 'Player identities are unavailable for these records.' : uiText("这些记录暂未提供可识别的选手信息。", uiLocale)}</p>}
    {hero.players.length > 6 ? <button className={styles.expandButton} type="button" aria-expanded={expanded} onClick={onToggle}>{expanded ? (isEn ? 'Show fewer players' : uiText("收起选手", uiLocale)) : (isEn ? `All ${hero.players.length} players` : uiText("查看全部 {0} 位选手", uiLocale, [hero.players.length]))}<Arrow down={!expanded} /></button> : null}
  </section>
}

function MapEvidence({ hero, isEn, locale, withSeason, evidenceHref, expanded, onToggle }) {
  const location = useLocation()
  return <section className={styles.analysisPanel} id="hero-maps" aria-labelledby="hero-maps-heading">
    <header><h3 id="hero-maps-heading">{isEn ? 'Where the hero appears' : uiText("在哪些地图留下记录", locale)}</h3><span>{hero.maps.length} {isEn ? (hero.maps.length === 1 ? 'map' : 'maps') : uiText("张地图", locale)}</span></header>
    ...
    <ol className={styles.mapList}>{hero.maps.slice(0, expanded ? undefined : 6).map(map => <li key={map.key}><Link className={styles.mapIdentity} to={withSeason(`/maps/${encodeURIComponent(map.routeName)}`)} state={getReturnState(location)} onClick={() => saveReturnScroll(location)}><AtlasImage src={mapImageUrl(map)} className={styles.mapThumb} /><span><strong>{formatOwMapName(map.name, locale)}</strong><small>{formatOwMapMode(map.type, locale)}</small></span></Link><Link className={styles.mapCount} to={evidenceHref({ heroMap: map.name })} aria-label={isEn ? `View ${map.count} ${map.count === 1 ? 'appearance' : 'appearances'} on ${map.name}` : uiText("查看在{0}的 {1} 次出场记录", locale, [formatOwMapName(map.name, locale), map.count])}><b>{map.count}</b><small>{percentage(map.rate)}</small><Arrow /></Link></li>)}</ol>
    {hero.maps.length > 6 ? <button className={styles.expandButton} type="button" aria-expanded={expanded} onClick={onToggle}>{expanded ? (isEn ? 'Show fewer maps' : uiText("收起地图", locale)) : (isEn ? `All ${hero.maps.length} maps` : uiText("查看全部 {0} 张地图", locale, [hero.maps.length]))}<Arrow down={!expanded} /></button> : null}
...
function RecordLedger({ hero, params, setParams, locale, isEn, withSeason }) {
  ...
  const visibleCount = Math.min(hero.records.length, Math.max(8, Number(params.get('heroRecordLimit')) || 8))
  ...
  const update = values => setParams(previous => {
    ...
    if (!Object.hasOwn(values, 'heroRecordLimit')) next.delete('heroRecordLimit')
  ...
  return <section id="hero-records" className={styles.records} aria-labelledby="hero-records-title">
    ...
    {records.length > visibleCount ? <button className={styles.expandButton} type="button" onClick={() => update({ heroRecordLimit: visibleCount + 8 })}>{isEn ? 'More matches' : uiText("继续查看", locale)}<span>{Math.min(visibleCount, records.length)} / {records.length}</span><Arrow down /></button> : null}
  </section>
}
...
export default function SignalHeroDossier({ guide, hero, params, setParams, locale, withSeason }) {
  ...
  const location = useLocation()
  const updateReading = values => setParams(previous => {
    const next = new URLSearchParams(previous)
    Object.entries(values).forEach(([key, value]) => value ? next.set(key, value) : next.delete(key))
    return next
  }, { replace: true, preventScrollReset: true, flushSync: true, state: { ...location.state, restoreScrollY: undefined } })
  ...
  const evidenceHref = filters => {
    ...
    ;['heroMap', 'heroPlayer', 'heroRecordSearch', 'heroRecordLimit'].forEach(key => next.delete(key))
  ...
  return <div className={styles.shell} data-hero-guide="dossier" data-role={hero.role} data-i18n-ignore>
    ...
    <header className={styles.dossierHero}>
      ...
      <div className={styles.dossierCopy}><span className={styles.eyebrow}>{roleLabel(hero.role, locale)}<i> / </i>{isEn ? `No. ${hero.roleRank} in recorded appearances for this role` : uiText("本职责记录出场第 {0}", locale, [hero.roleRank])}</span><h1>{formatOwHeroName(hero.name, locale)}</h1>{!isEn ? <span className={styles.dossierEnglish}>{hero.name.toUpperCase()}</span> : null}<div className={styles.dossierRate}><strong>{percentage(hero.rate)}</strong><span>{isEn ? 'recorded appearance rate' : uiText("记录出场率", locale)}<small>{hero.count} / {guide.samples} {isEn ? 'team-sides' : uiText("份队伍单图记录", locale)}</small></span></div></div>
      <span className={styles.dossierRole}>{hero.role.toUpperCase()}</span>
    </header>
    <div className={styles.dossierFacts}><p><strong>{hero.count}</strong><span>{isEn ? 'recorded appearances' : uiText("次记录出场", locale)}</span></p><a href="#hero-players"><strong>{hero.players.length}</strong><span>{isEn ? (hero.players.length === 1 ? 'recorded player' : 'recorded players') : uiText("位使用选手", locale)}</span><Arrow down /></a><a href="#hero-maps"><strong>{hero.maps.length}</strong><span>{isEn ? (hero.maps.length === 1 ? 'map with this hero' : 'maps with this hero') : uiText("张出场地图", locale)}</span><Arrow down /></a><a href="#hero-records"><strong>{hero.records.length}</strong><span>{isEn ? 'map records' : uiText("条可回溯地图记录", locale)}</span><Arrow down /></a></div>
