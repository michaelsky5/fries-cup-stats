...
const resultLabel = (result, en) => ({ win: en ? 'WIN' : '胜', loss: en ? 'LOSS' : '负', draw: en ? 'DRAW' : '平', pending: en ? 'LIVE' : '进行中', unknown: '—' })[result]
    ...
    {matches.map(match => {
      ...
      return <section className={styles.match} key={match.key} data-result={match.result}>
        <div className={styles.matchRow}>
          ...
          <div className={styles.result}><span>{resultLabel(match.result, en)}</span><strong>{score(match)}</strong></div>
          ...
          <div className={styles.mapScroll} role="region" aria-label={en ? 'Player map statistics' : uiText("选手逐图数据", locale)} tabIndex={0}>
            <table><thead><tr><th scope="col">{en ? 'Map / Hero' : uiText("地图 / 英雄", locale)}</th><th scope="col">{en ? 'Result' : uiText("赛果", locale)}</th><th scope="col">{en ? 'ELIM' : uiText("消灭", locale)}</th><th scope="col">{en ? 'AST' : uiText("助攻", locale)}</th><th scope="col">{en ? 'DTH' : uiText("阵亡", locale)}</th><th scope="col">{en ? 'Damage' : uiText("伤害", locale)}</th><th scope="col">{en ? 'Healing' : uiText("治疗", locale)}</th><th scope="col">{en ? 'Mitigation' : uiText("阻挡", locale)}</th><th scope="col">{en ? 'Rating' : uiText("评分", locale)}</th></tr></thead><tbody>
              {match.maps.map(map => <tr key={map.order} data-hero-match={(selectedHero || selectedMap) && (!selectedMap || playerMapKey(map.name) === selectedMap) && (!selectedHero || map.heroes.some(hero => getOwHeroCanonicalKey(hero) === selectedHero)) || undefined}>
                <th scope="row"><Link {...linkProps(`/matches/${encodeURIComponent(match.matchId)}?map=${map.order}`)}><span className={styles.mapIdentity}><small>{String(map.order).padStart(2, '0')}</small><strong>{map.name} ↗</strong></span><span className={styles.mapHeroes}>{map.heroes.map(hero => <SignalHeroPortrait key={hero} hero={hero} role={map.role} description={formatOwHeroName(hero, locale)} />)}</span></Link></th>
                <td className={styles.mapResult} data-result={map.result}><span>{resultLabel(map.result, en)}</span><b>{score(map)}</b></td>
                <td>{number(map.eliminations)}</td><td>{number(map.assists)}</td><td>{number(map.deaths)}</td><td>{number(map.damage)}</td><td>{number(map.healing)}</td><td>{number(map.mitigation)}</td><td><SignalRating value={map.rating} en={en} /></td>
              </tr>)}
            </tbody></table>
          </div>
        </div>}
