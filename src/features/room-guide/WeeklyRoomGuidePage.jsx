...
import { GUIDE_ROLES, GUIDE_SCENARIOS, GUIDE_VERSION, guideLessons, guidePracticeUrl, guideReturnPath, readGuideContext, roomGuideUrl } from './roomGuideModel.js'
...
export default function WeeklyRoomGuidePage() {
  ...
  return <main className={styles.page} data-i18n-ignore lang={locale}>
    <div className={styles.shell}>
      ...
      <div ref={simulatorRef}><RoomGuideSimulator key={context.role + ':' + context.mode + ':' + simulationScenario + ':' + simulationRun} role={context.role} mode={context.mode} scenario={simulationScenario} text={text} onFullCourse={role => { setSimulationScenario(''); select({ role, scenario: '', step: 'entry' }) }} /></div>
